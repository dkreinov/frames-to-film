...
| 7 | Quality: story-aware pipeline + judge-driven eval | pending | 6 | Story arc parameter + brief input + story writer + cinematic devices catalog + three-tier judge stack + eval harness with AI/human calibration + stitch polish. Built for paid-service stage (operator-driven, single tenant). 9 sub-plans (7.1–7.7 + 7.1.1 Olga-validation + 7.5b Kling-vs-Wan-2.7 generator A/B), ~14-19 focused days. See `phase_7_overview.md` and `phase_7_plan.md`. 7.1 ✓ done. 7.1.1 in progress (gates 7.2+). |
