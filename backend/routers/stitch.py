"""Stitch stage endpoint — final concat to full_movie.mp4."""
...
from backend.services import stitch as stitch_svc
...
def stitch(
    ...
    job_id = jobs_svc.create_job(
        db_path, project_id=project_id, user_id=user_id, kind="stitch", payload=payload
    )
    bg.add_task(jobs_svc.run_job_sync, db_path, job_id, stitch_svc.stitch_runner)
